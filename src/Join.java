...
public class Join {
	...
	public static class JoinMap2 extends Mapper<LongWritable, Text, Text, Text> {
		...
		// private final IntWritable one = new IntWritable(1);
		// private final IntWritable two = new IntWritable(2);
		private Logger logger = Logger.getLogger(JoinMap2.class);
		...
		protected void setup(Context context) throws IOException {
			...
			try{
				...
				logger.info("local cache => " + localfiles[0].toString());
				FileReader fr = new FileReader(localfiles[0].toString());
				BufferedReader br = new BufferedReader(fr);
				String inode;
				while (((inode = br.readLine()) != null)){
					logger.info("dir=>"+inode);
				}
			}catch (Exception e){
				logger.info("something wrong is here"+e.getMessage());
			}
	...
	public static class JoinReduce2 extends Reducer<Text, Text, Text, NullWritable> {
		...
		public void reduce(Text key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			else
			{
				...
				{
					if(result[i]==null)
						context.write(new Text("NULL"), null);
					else
